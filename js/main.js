...
// ...
    ...
    // Check if modal elements exist before defining handlers
    if (modal && form) {
        function openModal() {
            resetForm();
            modal.classList.add('show');
            document.body.style.overflow = 'hidden'; // Keep lock for modal
        }
        ...
        form.addEventListener('submit', function (e) {
            ...
            setTimeout(() => {
                // Form data processing (simulated)
    ...
    // =========================================================================
    // 7. SIMPLE SCROLL ANIMATIONS (Intersection Observer)
    // =========================================================================
    function initScrollAnimations() {
        const animateElements = document.querySelectorAll('.service-card, .choose-card, .project-item, .team-member');

        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.classList.add('animate'); // Add CSS class for animation
        ...
        animateElements.forEach(el => {
            // Apply initial CSS state if needed, or rely solely on the .animate class CSS
            observer.observe(el);
        });
    }
    ...
    // =========================================================================
    // 8. EXTERNAL ANIMATION INITIALIZATION (AOS/WOW)
    // =========================================================================
    // Since you are linking these libraries in your HTML, initialize them.
    ...
    if (typeof WOW !== 'undefined') {
        // WOW relies on the 'animate.css' classes you added to work.
        new WOW({
            offset: 100,
            mobile: true
        }).init();
